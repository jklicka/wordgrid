...
test('every asset the game needs offline is precached', async ({ page }) => {
  ...
  expect(urls.some((u) => /^\/assets\/index-.*\.js$/.test(u)), 'main bundle').toBe(true)
  expect(urls.some((u) => /^\/assets\/index-.*\.css$/.test(u)), 'stylesheet').toBe(true)
  ...
  const levelChunks = urls.filter((u) => /^\/assets\/level-\d+-.*\.js$/.test(u))
